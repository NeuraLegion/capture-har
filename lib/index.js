...
function transformAsciiToUtf8 (value) {
  return value !== undefined && value !== null
    ? Buffer.from(String(value), 'ascii').toString('utf8')
    : value;
}
  ...
  return Object.keys(obj).reduce((result, key) => {
    ...
    if (Array.isArray(value)) {
      return result.concat(value.map(v => ({
        name: key,
        value: transformAsciiToUtf8(v)
      })));
    } else {
      return result.concat({
        name: key,
        value: transformAsciiToUtf8(value)
      });
    }
  }, []);
}
...
function parseRedirectUrl (response) {
  if (response && response.statusCode >= 300 && response.statusCode < 400) {
    var location = transformAsciiToUtf8(response.headers['location']);
    if (location) {
