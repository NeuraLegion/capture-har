...
function buildHarEntry (request, error, response, harConfig, meta) {
  return {
    ...
    response: buildHarResponse(error, response, harConfig),
    cache: {},
    timings: {
      send: 0,
      receive: 0,
...
function captureEntry (requestConfig, harConfig) {
  ...
  return reqObject
    .then(response => {
      return [ reqObject, null, response ];
    }, error => {
      return [ reqObject, error.cause, error.response ];
    });
}
